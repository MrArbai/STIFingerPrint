...
Public Class FrmTrnApproveAbsensiByPSN : Inherits DockContent
    Private Sub FrmTrnApproveAbsensiByPSN_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Call SetGridDefult()
    ...
    Private Sub SetGridDefult()
        ...
        With fg1
            ...
            txt = .GetCellRange(0, 3, 0, 3) : txt.Data = "Kembali Ke Departemen" : .Cols(3).Name = "KembaliKeDepartemen" : .Cols(3).Width = 80 : .Cols(3).TextAlign = TextAlignEnum.CenterCenter : .Cols(3).DataType = GetType(Boolean)
            ...
            txt = .GetCellRange(0, 7, 0, 7) : txt.Data = "Diapprove Oleh" : .Cols(7).Name = "CreateBy" : .Cols(7).Width = 100 : .Cols(7).TextAlign = TextAlignEnum.CenterCenter
            txt = .GetCellRange(0, 8, 0, 8) : txt.Data = "Diapprove Tanggal" : .Cols(8).Name = "CreatedDate" : .Cols(8).Width = 130 : .Cols(8).TextAlign = TextAlignEnum.CenterCenter
            txt = .GetCellRange(0, 9, 0, 9) : txt.Data = "Komputer" : .Cols(9).Name = "Computer" : .Cols(9).Width = 100 : .Cols(9).TextAlign = TextAlignEnum.CenterCenter
            txt = .GetCellRange(0, 10, 0, 10) : txt.Data = "Komputer Tanggal" : .Cols(10).Name = "ComputerDate" : .Cols(10).Width = 100 : .Cols(10).TextAlign = TextAlignEnum.CenterCenter
    ...
    Private Sub GetDataHeader()
        Dim sql As String = ""
        Dim i As Integer = 1
        sql = "SELECT AbsenHdrID,StatusTk,a.DeptID,b.Departemen,PeriodeTgl,PeriodeBln,a.Approve1, " &
            " CASE WHEN a.CompleteStatus=1 THEN 'SUDAH KOMPLIT' ELSE 'BELUM KOMPLIT' END AS StatusComplete,a.Approve1Date,UpdatedBy,UpdatedDate,a.Computer,a.ComputerDate  " &
            " FROM tblTrnAbsensiHdrKry A JOIN vwMstDept b ON a.DeptID=b.DeptID  where a.DeptID='" & cboDepartemen.SelectedValue & "' and a.PeriodeBln='" & Format(dtPeriode1.Value, "yyyy-MM-01") & "' and Approve1 is not null Order By PeriodeTgl "
                ...
                If DR.HasRows Then
                    With fg1
                        While DR.Read
                            ...
                            .Item(i, "CreateBy") = DR!Approve1
                            .Item(i, "CreatedDate") = DR!Approve1Date
    ...
    Private Sub fg1_AfterEdit(sender As Object, e As RowColEventArgs) Handles fg1.AfterEdit
        With fg1
            If .Item(e.Row, "Approve") = True Then
                .Item(e.Row, "KembaliKeDepartemen") = False

            End If
            If .Item(e.Row, "KembaliKeDepartemen") = True Then
    ...
    Private Sub Simpan()
        ...
        Dim ListAbsenHdrIDToDept As String = ""
        Try
            For i = 1 To fg1.Rows.Count - 1
                If fg1.Item(i, "Approve") = True Then
                    ...
                    End If
                ElseIf fg1.Item(i, "KembaliKeDepartemen") = True Then
                    If ListAbsenHdrIDToDept = "" Then
                        ListAbsenHdrIDToDept = fg1.Item(i, "AbsenHdrID")
                    Else
                        ListAbsenHdrIDToDept = ListAbsenHdrIDToDept & "," & fg1.Item(i, "AbsenHdrID")
                    End If
                End If
            Next
            If ListAbsenHdrIDToDept = "" And ListAbsenHdrID = "" Then
            ...
            With CMD
                ...
                If ListAbsenHdrID <> "" Then
                    .CommandText = "Update tblTrnAbsensiHdrKry set Approve2='" & UserLogin & "',Approve2Date=getdate() where AbsenHdrID in (" & ListAbsenHdrID & ") "
                    .ExecuteNonQuery()
                End If
                If ListAbsenHdrIDToDept <> "" Then
                    .CommandText = "Update tblTrnAbsensiHdrKry set Approve1=null,Approve1Date=null where AbsenHdrID in (" & ListAbsenHdrIDToDept & ") "
                    .ExecuteNonQuery()
                End If
            End With
